<template>
  <div class='calendar'>
    ...
    <div  class='calendar-item' v-for='(item) in calendarInfo' :key="item.id">
      <h3 class='border-bottom' ref="heaven">{{item.calendarInfo.title}}</h3>
      <div class="calendar-info border-bottom"  @click='handleClick(result)' v-for='result in item.calendarInfo.result' :key="result.id">
